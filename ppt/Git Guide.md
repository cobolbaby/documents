...
分布式版本控制系统，每个人的电脑上都有一个完整的版本库，这样，你工作的时候就不需要联网了; 而“中央服务器”的作用仅仅是用来方便“交换”大家的修改，没有它大家也一样干活，只是交换修改不方便而已。而Git作为其中的代表优势不单是不必联网这么简单，后面我们还会看到Git极其强大的分支管理，把SVN等远远抛在了后面。

--

### 分布式版本控制系统

分布式版本控制系统中我们打交道最多的是本地仓库，下面主要讲一下本地仓库中的相关内容。

以宏观的视角来看，Git本地仓库可以分为三个区(三颗"树")
...
开发者可通过多个本地分支在不同任务之间进行快速切换，并且不需要像SVN那样切目录
